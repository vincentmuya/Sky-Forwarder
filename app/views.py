...
def newcargo(request):
    ...
    ScheduledFlight = request.POST.get('ScheduledFlight')
    ...
    InTransit = request.POST.get('InTransit')
    GoodsDescription = request.POST.get('GoodsDescription')
    Status = request.POST.get('Status')

    recipient=NewForm(SenderName=SenderName, SenderAddress=SenderAddress, RecieverName=RecieverName, RecieverAddress=RecieverAddress, referenceID=referenceID, Origin=Origin, Destination=Destination,ScheduledFlight=ScheduledFlight, DepatureDate=DepatureDate, Depaturetime=Depaturetime, ArrivalDate=ArrivalDate, ArrivalTime=ArrivalTime,Courier=Courier, InTransit=InTransit, GoodsDescription=GoodsDescription, Status=Status)
    recipient.save()
...
def new_secure(request):
    ...
    recipient=Secure(DepositorName=DepositorName, ReceiverName=ReceiverName, TrackNo=TrackNo, Origin=Origin, Destination=Destination,ScheduledFlight=ScheduledFlight,InTransit=InTransit,TypeOfShipment=TypeOfShipment, NatureOfGoods=NatureOfGoods, Status=Status)
    recipient.save()
...
def search_secure(request):
    ...
    if 'TrackNo' in request.GET and request.GET["TrackNo"]:
        ...
        message = "You haven't searched for any term"
        return render(request, 'secure_search.html',{"message":message})
